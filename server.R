...
server <- function(input, output) {
  ...
  nv <-  reactive({
        calc_weights(inputs(), nv_weights)
    })
  
  v <-  reactive({
        calc_weights(inputs(), v_weights)
    })
  
  
  # Generates outputs for display
  ...
  output$deprPlot <- renderPlot({
    ...
    ggplot(data.frame(x = 0:209), aes(x)) +
      geom_function(fun = function(x) 
        nv()[1] +
          nv()[2]*ifelse(x>74, 74, x) +
          nv()[3]*ifelse(x>74, 74, x)^2 +
          nv()[4]*ifelse(x<75, 0, x-75) +
          nv()[5]*ifelse(x<75, 0, x-75)^2 +
          nv()[6]*ifelse(x<75, 0, x-75)^3,
                    aes(color = "Non-Victoria")
      ) +
      geom_function(fun = function(x) 
        v()[1] +
          v()[2]*ifelse(x>74, 74, x) +
          v()[3]*ifelse(x>74, 74, x)^2 +
          v()[4]*ifelse(x<75, 0, x-75) +
          v()[5]*ifelse(x<75, 0, x-75)^2 +
          v()[6]*ifelse(x<75, 0, x-75)^3,
                    aes(color = "Victoria")
      ) +
      #experimental annotation layers Lockdown 7th July to 26th October
      geom_vline(xintercept = 92, colour = 'red') + 
      annotate("text", x = 93, y = 12, hjust = 0,
               label = "Melbourne enters \nsecond lockdown") + 
      geom_vline(xintercept = 203, colour = 'red') + 
      annotate("text", x = 202, y = 12, hjust = 1,
               label = "Melbourne exits \nsecond lockdown") +
      
      coord_cartesian(xlim = c(0, 200), ylim = c(0, 18)) +
      theme_classic() +
      ...
      labels = as.character(lubridate::dmy("6th April, 2020") + 0:209)[(c(0, 50, 100, 150, 200)+1)]
      ) +
      theme(line = element_line(size = 1.5),
            text = element_text(size = 16),
            panel.grid.major.y = element_line(colour = "grey80", size = .3)) + 
      labs(y = "Depression", x = "Days since April 6th", color = "Legend")
